#!/bin/bash
# Resubmit a large-batch job, resuming from latest checkpoint
# Usage: bash scripts/resubmit_lb.sh [config_name] [iterations]
#   e.g. bash scripts/resubmit_lb.sh noise-hidden6-ema-lb
#   e.g. bash scripts/resubmit_lb.sh noise-hidden6-ema-lb 160000
...
if [ -z "$LATEST_CKPT" ]; then
    ...
    bash scripts/submit_lb.sh "$CONFIG_NAME"
    exit 0
fi
