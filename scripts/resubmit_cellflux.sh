#!/bin/bash
# Resubmit a CellFlux job, resuming from latest checkpoint
# Usage: bash scripts/resubmit_cellflux.sh [config_name] [iterations]
#   e.g. bash scripts/resubmit_cellflux.sh cellflux-bbbc021-lb
#   e.g. bash scripts/resubmit_cellflux.sh cellflux-bbbc021-paper-lb 160000

set -e

CONFIG_NAME="${1:-cellflux-bbbc021-lb}"
ITERATIONS="${2:-}"
...
if [ -z "$LATEST_CKPT" ]; then
    ...
    bash scripts/submit_cellflux.sh "$CONFIG_NAME"
    exit 0
fi
