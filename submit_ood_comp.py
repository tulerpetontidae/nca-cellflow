"""Submit 96px OOD comparison jobs (NCA, NCA-inter, CellFlux, IMPA) via bsub.

All models train with 8 compounds excluded (OOD split), evaluate FID + MoA
on all 34 compounds every 5k steps.

Usage:
    python submit_ood_comp.py                  # submit all jobs
    python submit_ood_comp.py --resume         # auto-find latest checkpoint
    python submit_ood_comp.py --only nca       # submit only jobs matching "nca"
    python submit_ood_comp.py --dry-run        # print commands without submitting
"""
...
configs = [
    # --- NCA (LatentNCA) 96px OOD ---
    {
        "model": "nca",
        "wandb_name": "nca-h24-nh256-w96D-96px-ood",
        "config": "configs/latent-h24-nh256-w96D-96px-ood-lb.yaml",
        "checkpoint_dir": f"{_ckpt_prefix}/latent-h24-nh256-w96D-96px-ood-lb",
    },
    # --- NCA + intermediate regularization ---
    {
        "model": "nca",
        "wandb_name": "nca-h24-nh256-w96D-inter-96px-ood",
        "config": "configs/latent-h24-nh256-w96D-inter-96px-ood-lb.yaml",
        "checkpoint_dir": f"{_ckpt_prefix}/latent-h24-nh256-w96D-inter-96px-ood-lb",
    },
    # --- CellFlux 96px OOD ---
    {
        "model": "cellflux",
        "wandb_name": "cellflux-96px-ood",
        "config": "configs/cellflux-bbbc021-96px-ood-lb.yaml",
        "checkpoint_dir": f"{_ckpt_prefix}/cellflux-bbbc021-96px-ood-lb",
    },
    # --- IMPA 96px OOD ---
    {
        "model": "impa",
        "wandb_name": "impa-96px-ood",
        "config": "configs/impa-bbbc021-96px-ood-lb.yaml",
        "checkpoint_dir": f"{_ckpt_prefix}/impa-bbbc021-96px-ood-lb",
    },
]
